...
export default function App() {
  ...
  const sectorMap = { sector_edu: "q3_1", sector_health: "q3_2", sector_fin: "q3_3", sector_govt: "q3_4" };
      ...
      <div style={styles.app}>
        ...
        <div style={styles.main}>
          <div style={styles.intro}>
            <div style={styles.introTitle}>Does your AI system comply with Indian regulations?</div>
            <div style={styles.introSub}>
              India's AI governance landscape is evolving fast. This tool maps your AI system against the DPDP Act 2023, MeitY AI Governance Guidelines 2025, NITI Aayog's Responsible AI Principles, and sector-specific frameworks — and tells you what obligations apply.
              <br /><br />
              <strong>5 modules · ~20 questions · ~5 minutes.</strong>
            </div>

            <div style={{ fontSize: 12, fontWeight: 700, color: "#64748b", marginBottom: 12, letterSpacing: 1, fontFamily: "'Courier New', monospace" }}>YOUR ORGANISATION TYPE</div>
            <div style={styles.orgGrid}>
              {ORG_TYPES.map((o) => (
                <div
                  key={o.id}
                  style={{ ...styles.orgCard, ...(orgType === o.id ? styles.orgCardActive : {}) }}
            ...
            <button
              style={{ ...styles.startBtn, opacity: orgType ? 1 : 0.4, cursor: orgType ? "pointer" : "not-allowed" }}
              disabled={!orgType}
              onClick={() => { if (orgType) setPhase("quiz"); }}
            >
              Start Assessment →
            </button>

            <div style={{ marginTop: 20, padding: "12px 16px", background: "#fff7ed", borderRadius: 8, border: "1px solid #fed7aa" }}>
              <div style={{ fontSize: 11, color: "#92400e", lineHeight: 1.5 }}>
                <strong>⚠️ Disclaimer:</strong> This tool provides general guidance only and does not constitute legal advice. India's AI regulatory framework is evolving — consult a qualified legal professional before making compliance decisions.
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={styles.app}>
        ...
        <div style={styles.main}>
          <div style={styles.progressRow}>
            <div style={styles.progressLabel}>Question {answeredSoFar + 1} of {totalQ}</div>
          ...
          <div style={styles.moduleBar}>
            {MODULES.map((m, i) => (
              <div
                ...
                style={{
                  ...styles.modulePill,
                  background: i < currentModule ? "#0f2d5c" : i === currentModule ? "#f59e0b" : "#e2e8f0"
                }}
              />
            ))}
          </div>
          ...
          <div style={styles.questionCard}>
            <div style={{ ...styles.moduleTag, background: modBg, color: modColor }}>
              {curModule.label} · {curModule.title.toUpperCase()}
            </div>
            <div style={styles.questionText}>{curQuestion.text}</div>
            <div style={styles.lawRef}>📋 {curQuestion.law}</div>

            {curQuestion.def && (
              <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderLeft: `3px solid ${modColor}`, borderRadius: 8, padding: "12px 14px", marginBottom: 20 }}>
                <div style={{ fontSize: 10, fontWeight: 800, color: modColor, letterSpacing: 1, marginBottom: 6, fontFamily: "'Courier New', monospace" }}>📖 DEFINITION & SCOPE</div>
                <div style={{ fontSize: 12, color: "#374151", lineHeight: 1.7 }}>{curQuestion.def}</div>
              </div>
            )}
          ...
          <div style={{ background: "#f0f5ff", borderRadius: 10, padding: "12px 16px", border: "1px solid #bfdbfe", marginBottom: 16 }}>
            <div style={{ fontSize: 11, color: "#1e40af", lineHeight: 1.5 }}>
              <strong>Why this matters:</strong> {curQuestion.yes.note}
            </div>
          </div>

          <div style={styles.navRow}>
            <button style={styles.backBtn} onClick={goBack} disabled={currentModule === 0 && currentQ === 0}>
              ← Back
            </button>
    ...
    return (
      <div style={styles.app}>
        ...
        <div style={styles.main}>
          ...
          <div style={{ ...styles.resultHeader, borderLeft: `5px solid ${riskColor}` }}>
            <div style={{ ...styles.riskBadge, background: riskColor, color: "white" }}>
              ...
              {flaggedIssues.map((q) => {
                ...
                return (
                  <div key={q.id} style={styles.issueItem}>
                    ...
                    <div style={{ fontSize: 10, color: "#b45309", fontFamily: "'Courier New', monospace", marginTop: 4 }}>{q.law}</div>
                  </div>
                );
              })}
            </div>
          ...
          {activeSectorsList.length > 0 && (
            <div style={styles.section}>
              ...
              {activeSectorsList.map((s) => {
                ...
                return (
                  <div key={s} style={{ ...styles.sectorCard, borderLeft: `3px solid ${sr.color}` }}>
          ...
          <div style={styles.section}>
            ...
            {obligations.map((ob, i) => (
              <div key={i} style={{ ...styles.oblItem, borderBottom: i < obligations.length - 1 ? "1px solid #f1f5f9" : "none" }}>
                <div style={{ ...styles.oblDot, background: ob.required ? riskColor : "#94a3b8" }} />
                <div style={{ flex: 1 }}>
                  <div style={styles.oblText}>{ob.text}</div>
                  {ob.law && <div style={styles.oblLaw}>{ob.law}</div>}
                  <div style={{ ...styles.oblRequired, color: ob.required ? "#dc2626" : "#94a3b8" }}>
            ...
            <div style={styles.section}>
              ...
              {hasGovtFlag && (
                <div style={{ padding: "10px 14px", background: "#f5f3ff", border: "1px solid #ddd6fe", borderRadius: 8, marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#6b21a8", lineHeight: 1.5 }}>Wider exemptions apply for national security and public order. However, constitutional safeguards (proportionality test) still apply to all AI use, and the Supreme Court's mandated safeguards for surveillance remain in force.</div>
                </div>
              )}
              {!hasDPDPFlag && (
                <div style={{ padding: "10px 14px", background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#166534", marginBottom: 4 }}>DPDP Act May Not Apply</div>
                  <div style={{ fontSize: 12, color: "#15803d", lineHeight: 1.5 }}>If you confirmed your data is not in digital form, the DPDP Act 2023 does not directly apply. However, NITI Aayog's voluntary Responsible AI principles and MeitY Guidelines still apply as guidance — especially if you work with government programmes.</div>
                </div>
              )}
            </div>
          )}
          ...
          <div style={styles.section}>
            ...
            {[
              ...
              <div key={i} style={{ fontSize: 12, color: "#64748b", padding: "7px 0", borderBottom: i < 4 ? "1px solid #f1f5f9" : "none", lineHeight: 1.4 }}>
                → {gap}
              </div>
            ))}
          </div>
          ...
          <div style={styles.disclaimer}>
            <div style={styles.disclaimerText}>
              <strong>⚠️ Disclaimer:</strong> This tool provides general guidance based on India's AI regulatory landscape as of February 2026. It does not constitute legal advice. The framework is evolving — consult a qualified legal professional before making compliance decisions. Sources: DPDP Act 2023 + Rules 2025 · MeitY AI Governance Guidelines (Nov 2025) · NITI Aayog Responsible AI (2021) · RBI FREE-AI Framework (Aug 2025) · IT Rules 2021 · MeitY Advisory 2024.
            </div>
          </div>
